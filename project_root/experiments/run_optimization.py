...
# ---------------------------------------------------------------------------
# Two reference image sets are available:
#  - 'synthetic'  : reproducible 2D microstructure PNGs generated by
#                   make_synthetic_targets.py. Ground-truth parameters are
#                   recorded in exp_img_synthetic/ground_truth.json. Citable.
#                   Default for new runs.
#  - 'consortium' : private ESFR-SIMPLE WP8 micrographs (connected_79.png,
#                   distributed_77.png). Used to produce the original Table 1
#                   numbers. Cannot be reproduced in journal publication
#                   without rights clearance from the consortium.
_CONSORTIUM_BASE = Path("/home/giovanni/Merope/Optimization_3D_structure/exp_img")
_SYNTHETIC_BASE  = Path("/home/giovanni/Merope/Optimization_3D_structure/exp_img_synthetic")
_EXP_IMAGE_SETS = {
    "synthetic": {
        "distributed":    str(_SYNTHETIC_BASE / "synthetic_distributed.png"),
        "interconnected": str(_SYNTHETIC_BASE / "synthetic_interconnected.png"),
    },
    "consortium": {
        "distributed":    str(_CONSORTIUM_BASE / "distributed_77.png"),
        "interconnected": str(_CONSORTIUM_BASE / "connected_79.png"),
    },
}
...
def _make_space_interconnected() -> List:
    ...
    """
    return [
        Real(0.20, 1.5, name="delta"),          # Tightened 2026-05-05: upper
                                                 # bound 3.0 allowed the BO to
                                                 # land on degenerate corners
                                                 # (delta so wide the GB band
                                                 # spans most of the volume,
                                                 # making clipping inactive).
                                                 # 1.5 caps delta* at 0.5, the
                                                 # physically meaningful end of
                                                 # the interconnected regime.
        Real(0.05, 0.50, name="pore_phi"),      # Pore volume fraction
        Real(0.30, 0.50, name="pore_radius"),   # Tightened 2026-05-05: lower
                                                 # bound 0.20 let the BO drive
                                                 # radius toward zero in
                                                 # combination with thick delta;
                                                 # 0.30 keeps pores resolvable
                                                 # against the GB band.
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Explicit path to a reference image (overrides --exp-image-set)."
    )
    parser.add_argument(
        "--exp-image-set", choices=["synthetic", "consortium"], default="synthetic",
        dest="exp_image_set",
        help="Which reference image set to use (default: synthetic). "
             "'synthetic' uses reproducible images from make_synthetic_targets.py; "
             "'consortium' uses the private ESFR-SIMPLE WP8 micrographs."
    )
    ...
    mode = args.mode
    exp_image = args.exp_image or _EXP_IMAGE_SETS[args.exp_image_set].get(mode)
