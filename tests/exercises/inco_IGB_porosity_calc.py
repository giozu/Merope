...
inclPhi = np.linspace(0.02, 0.3, 20)
...
def Crack_structure_Voxellation(n3D, L, seed, inclR, inclPhi, lagRphi, incl_phase, grains_phase, delta_phase, delta, voxel_rule, K, vtkname, fileCoeff):
...
def scrivi_valori_su_file(file_output, valori, porosity, seed):
    media = sum(valori) / len(valori)
    num_decimali = len(str(valori[0]).split('.')[1]) if '.' in str(valori[0]) else 0
    media_formattata = f"{media:.{num_decimali}f}"
    with open(file_output, 'a') as f:
        linea = f"Porosity_{porosity:.2f}\tSeed_{seed}\t{valori[0]}\t{valori[1]}\t{valori[2]}\t{media_formattata}\n"
        f.write(linea)
...
def aggiorna_file_aggregato(file_aggregato, porosity, seed, valori, inclR, lagR, RVEsize, n3D, Kmatrix, Kgases):
    media = sum(valori) / len(valori)
    num_decimali = len(str(valori[0]).split('.')[1]) if '.' in str(valori[0]) else 0
    ...
    with open(file_aggregato, 'a') as f:
        linea = f"{porosity:.4f}\t{seed}\t{valori[0]:.4f}\t{valori[1]:.4f}\t{valori[2]:.4f}\t{media_formattata:.4f}\n"
        f.write(linea)
    ...
    for i in range(len(inclPhi)):
        for seed in range(NbSeed):
            ...
            porosity = Crack_structure_Voxellation(
                n3D, L, seed, inclR, inclPhi[i], lagRphi, incl_phase, grains_phase,
