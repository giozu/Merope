...
''' P = 0.1
num_iter = 21
inclPhi = [0.421, 0.3, 0.24, 0.201, 0.175, 0.157, 0.144, 0.134, 0.127, 0.121, 0.117, 0.1125, 0.11, 0.108, 0.105, 0.103, 0.101, 0.1, 0.099, 0.099, 0.099]
'''
porosity = 0.2
num_iter = 21
a = [0.421, 0.3, 0.24, 0.201, 0.175, 0.157, 0.144, 0.134, 0.127, 0.121, 0.117, 0.1125, 0.11, 0.108, 0.105, 0.103, 0.101, 0.1, 0.099, 0.099, 0.099]
inclPhi = [porosity*10*x for x in a]
delta = np.linspace(0.394, 3, num_iter) # thickness of layer POSSIBLE INPUT
...
def scrivi_valori_su_file(file_output, valori, porosity, delta, seed):
    media = sum(valori) / len(valori)
    num_decimali = len(str(valori[0]).split('.')[1]) if '.' in str(valori[0]) else 0
    media_formattata = f"{media:.{num_decimali}f}"
    with open(file_output, 'a') as f:
        linea = f"Porosity_{porosity:.2f}\tdelta_{delta}\tSeed_{seed}\t{valori[0]}\t{valori[1]}\t{valori[2]}\t{media_formattata}\n"
        f.write(linea)
...
def aggiorna_file_aggregato(file_aggregato, porosity, delta, seed, valori, inclR, lagR, RVEsize, n3D, Kmatrix, Kgases):
    media = sum(valori) / len(valori)
    num_decimali = len(str(valori[0]).split('.')[1]) if '.' in str(valori[0]) else 0
    ...
    if not os.path.exists(file_aggregato):
        with open(file_aggregato, 'w') as f:
            ...
            f.write("Porosity\tdelta\tSeed_index\tK_xx\tK_yy\tK_zz\tK_mean\n")  # Intestazione delle colonne

    # Aggiungi i risultati
    with open(file_aggregato, 'a') as f:
        linea = f"{porosity:.4f}\t{delta:.2f}\t{seed}\t{valori[0]:.4f}\t{valori[1]:.4f}\t{valori[2]:.4f}\t{media_formattata:.4f}\n"
        f.write(linea)
...
def main():
    ...
    for phi, d in zip(inclPhi, delta):
        for seed in range(NbSeed):
            ...
            scrivi_valori_su_file(file_output_path, valori_estratti, porosity, d, seed)

            # Aggiungi i risultati al file aggregato
            aggiorna_file_aggregato(file_aggregato, porosity, d, seed, valori_estratti, inclR, lagR, RVEsize, n3D, Kmatrix, Kgases)
