...
| `make_synthetic_targets.py` (new 2026-05-04) | Generates two reproducible 2D microstructure PNGs as drop-in replacements for the private ESFR-SIMPLE consortium reference images. Distributed = isolated RSA spheres; interconnected = Laguerre + δ=1 GB band + Boolean clipped pores ∪ independent intra RSA spheres. Realised porosities reproduce the paper's pore-analysis values by construction (p_b≈0.137, p_intra≈0.080, p_distributed≈0.230). | None (constants in file) | `Optimization_3D_structure/exp_img_synthetic/synthetic_{distributed,interconnected}.png`, `ground_truth.json` |
